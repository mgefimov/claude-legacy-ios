...
var LegacyTranspiler = (() => {
  ...
  async function transformDynamicImports(ast) {
    const allImports = [];
    simple(ast, {
    ...
    const literalImports = allImports.filter((node) => {
      const isLiteral = node.source.type === "Literal" && typeof node.source.value === "string";
      if (!isLiteral) {
        console.warn("[resolveDynamicImport] skipping non-literal import(".concat(node.source.type, ")"));
      }
      return isLiteral;
    });
  ...
  function dynamicImportCall(source, start, end) {
    return {
      type: "CallExpression",
      callee: {
        type: "MemberExpression",
  ...
  function transformImportMeta(ast, options2) {
    simple(ast, {
  ...
  var LOOKBEHIND_RE = /\(\?<[=!]([^()]*(?:\([^()]*\))*[^()]*)\)/g;
  function transformLookbehind(ast) {
    simple(ast, {
      Literal(node) {
        if (node.regex) {
  ...
  function transformStaticBlocks(ast) {
    counter = 0;
    simple(ast, {
